...
RSpec.describe 'Planning manager collaboration', type: :request do
  ...
  it 'fails closed when its School active Classroom context is injected' do
  ...
  it 'cannot read its School archive' do
  ...
  it 'cannot manage active Teachers or reissue their credentials' do
    active_teacher = create(:user, :teacher, school_year: active_year, school_role: 'member',
    ...
    end.not_to(change { active_year.users.teacher.count })
    ...
    end.not_to(change { active_teacher.teacher_credential_events.temporary_password_reissued.count })
    expect(response).to have_http_status(:not_found)
  ...
  it 'cannot manage active Classrooms or Students in its School' do
    ...
    expect(response).to redirect_to(root_path)
    expect(active_classroom.reload.class_label).not_to eq('수정 운영')
    ...
    expect(response).to redirect_to(root_path)
    expect(student.reload.name).not_to eq('수정 학생')

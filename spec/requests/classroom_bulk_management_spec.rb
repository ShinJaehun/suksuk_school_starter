...
RSpec.describe 'Classroom bulk management', type: :request do
  ...
  let(:manager) do
    create(:user, :teacher, school_year: active_year, school_role: 'manager', login_id: 'class-bulk-manager')
  end
  let(:planning_manager) do
    create(:user, :teacher, school_year: planning_year, school_role: 'manager', login_id: 'planning-class-bulk-manager')
  end
  ...
  it 'lets the current manager select own active and planning bulk contexts' do
    sign_in manager
  ...
  it 'limits the planning manager bulk context to its planning year' do
    ...
    get bulk_setup_admin_classrooms_path, params: context(planning_year)
    expect(response).to have_http_status(:ok)

    get bulk_setup_admin_classrooms_path, params: context(active_year)

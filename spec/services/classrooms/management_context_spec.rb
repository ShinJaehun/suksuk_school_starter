...
RSpec.describe Classrooms::ManagementContext do
  ...
  it 'gives an admin all years for an explicitly selected School and defaults to its active year' do
    ...
    expect(management_context).not_to be_read_only
  ...
  it 'lets a current manager select active, immediate planning, and archived years in its School' do
    ...
    expect(context(manager, school_year_id: planning_year.id)).to be_creatable

    archived_context = context(manager, school_year_id: archived_year.id)
    expect(archived_context).to be_read_only
    expect(archived_context).not_to be_creatable
    expect(archived_context.show_context_params).to eq(
      school_id: school.id,
  ...
  it 'limits a planning manager to its own planning year and uses it as the default' do
    ...
    expect do
      context(planning_manager, school_year_id: archived_year.id).selected_school_year
    end.to raise_error(ActiveRecord::RecordNotFound)
  end

  it 'marks archived and inactive-School contexts read-only' do
    admin = create(:user, :admin)
    school = create(:school)
    create(:school_year, :active, school: school, year: 2026)
    archived_year = create(:school_year, :archived, school: school, year: 2025)
    archived_context = context(admin, school_id: school.id, school_year_id: archived_year.id)

    expect(archived_context).to be_read_only
    expect(archived_context).not_to be_creatable

    school.update!(active: false)
    inactive_context = context(admin, school_id: school.id)
    expect(inactive_context).to be_read_only
    expect(inactive_context).not_to be_creatable
  end
  ...
  it 'resolves active and planning creation contexts but rejects archived creation' do
    ...
    expect(context(admin).creation_school).to be_nil

...
RSpec.describe TeacherManagementPolicy do
  ...
  it 'allows planning Teacher settings only in an authorized next-year context' do
    ...
    expect(described_class.new(planning_manager, member).update_profile?).to eq(false)
    expect(
  ...
  it 'limits an eligible active planning manager to its planning Teacher scope' do
    active_year = manager.school_year
